...
variable "subnet__virtual_network_data" {
  description = "Information of the virtual network to retrieve its data object (usually something like name='NNANPSpoke', resource_group_name='AzureVnet')."
  type = object({
...
variable "workspace__managed_resource_group_name" {
  ...
  validation {
    condition     = can(regex("[a-zA-Z0-9-._\\(\\)]+[a-zA-Z0-9-_\\(\\)]$", var.workspace__managed_resource_group_name))
    error_message = "Databricks Workspace managed resource group name must comply to regex [a-zA-Z0-9-._\\(\\)]+[a-zA-Z0-9-_\\(\\)]$."
  }

  validation {
    condition     = 1 <= length(var.workspace__managed_resource_group_name) && length(var.workspace__managed_resource_group_name) <= 90
    error_message = "Databricks Workspace managed resource group name must be between 1 and 90 characters."
  }
}
...
variable "workspace__secure_cluster_connectivity" {
  ...
  type        = bool
  default     = true # Read more at https://docs.microsoft.com/en-us/azure/databricks/security/secure-cluster-connectivity"
}
