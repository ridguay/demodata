resource "azurerm_key_vault" "key_vault" {
  ...
  network_acls {
    default_action             = "Deny"
    bypass                     = var.key_vault_enable_disk_encryption || var.network_control_allowed_azure_services ? "AzureServices" : "None"
...
resource "azurerm_private_endpoint" "network" {
  ...
  location            = var.key_vault_location
  name                = local.private_endpoint_name_prefix
  resource_group_name = var.resource_group_name
  subnet_id           = var.private_endpoint_subnet_id
  tags                = var.tags

  private_service_connection {
    name                           = local.private_endpoint_service_connection_name_prefix
