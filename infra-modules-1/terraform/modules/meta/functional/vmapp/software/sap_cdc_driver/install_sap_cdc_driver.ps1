<#
.SYNOPSIS
	This Powershell script installs SAP CDC driver on the Shared Intergration Runtime VM's
...
function Install-SAP-CDC-Driver([string] $sapPath) {
    if ( [string]::IsNullOrEmpty($sapPath)) {
        Throw-Error "SAP CDC Driver package path is not specified"
    }

    if (!(Test-Path -Path $sapPath)) {
        Throw-Error "Invalid SAP CDC Driver path: $sapPath"
    }

    $msipath = $sappath + "\NCo3026_Net40_x64.msi"

    Trace-Log "Start SAP CDC Driver installation"
    # /GAC_WMI=2 selects the correct installation option choice.
    # This was found using this https://stackoverflow.com/questions/7302251/determine-if-msi-exe-supports-certain-flag-argument
    Run-Process "msiexec.exe" "/i $msipath /quiet /qn /norestart GAC_WMI=2"
}

function Install-vcredist-Runtime([string] $sapPath) {
    if ( [string]::IsNullOrEmpty($sapPath)) {
        Throw-Error "SAP CDC Driver package path is not specified"
    }
    ...
    Trace-Log "Start SAP CDC Driver installation"
    Run-Process "$sapPath\vcredist_x64.exe" '/q'
}
...
Expand-Archive "$Package_Folder\$StorageFilename" -DestinationPath $Package_Folder -force

Install-vcredist-Runtime -sapPath $Package_Folder
Install-SAP-CDC-Driver -sapPath $Package_Folder
