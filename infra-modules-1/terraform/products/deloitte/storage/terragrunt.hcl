...
terraform {
  source = "${get_env("TERRAGRUNT_TERRAFORM_DIR")}/modules/deloitte_storage///"
}

locals {
  container_names = ["codeconverter"]

  roles_deloitte_resource_group = {
    for name, object_id in include.root.locals.domain_variables.object_ids.deloitte :
    object_id => ["Reader"]
  }

  roles_deloitte_storage = {
    for name, object_id in include.root.locals.domain_variables.object_ids.deloitte :
...
inputs = {
  ...
  resource_group_role_assignment_vars = {
    principal_id_roles = local.roles_deloitte_resource_group
  }

  storage_role_assignment_vars = {
    principal_id_roles = local.roles_deloitte_storage
  }
